...
3. QQ 空间偶发性存在随机性不能滚动 + crash 的问题.
4. 部分表情, 空间中的图片随机性无法加载.
5. 输入法在粘贴图片后小概率失效, 无法稳定复现. 可以通过切换聊天窗口来恢复.
6. 只在 archlinux + i3wm 上进行过测试.
7. 无法拖动传文件, 未来可以通过软链接容器内 shared 文件夹到一个与宿主机 shared 文件夹路径相同的文件夹来解决.
